...
// ─── Editable image (reused from club info) ──────────────────────────────────

function EditableImage({
  src, fallbackSrc, alt, fill, width, height, className, editable, onUploaded, folder, rounded,
}: {
  src: string; fallbackSrc: string; alt: string; fill?: boolean; width?: number; height?: number
  className?: string; editable: boolean; onUploaded: (key: string) => void
  folder: "covers" | "logos" | "avatars" | "documents"; rounded?: boolean
}) {
  ...
  return (
    <div className={`relative cursor-pointer group ${rounded ? "rounded-full overflow-hidden" : ""}`}
      style={fill ? { position: "absolute", inset: 0 } : undefined}
      onClick={() => !uploading && inputRef.current?.click()}>
      {imageEl}
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center">
        {uploading ? <div className="w-8 h-8 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Camera className="w-7 h-7 text-white opacity-0 group-hover:opacity-100 transition" />}
      </div>
...
function FormSelector({ organizationId, selectedId, onSelect }: { organizationId: number; selectedId: number | null; onSelect: (id: number | null) => void }) {
  const { data } = useQuery({ queryKey: ["forms", { organizationId }], queryFn: () => getFormList({ organizationId, limit: 50 }), enabled: !!organizationId })
  const forms: Form[] = (data?.data as any)?.data ?? []
  return (
    <div className="space-y-1 max-h-48 overflow-y-auto border rounded-lg p-2">
      <div className={`px-3 py-2 rounded cursor-pointer text-sm ${selectedId === null ? "bg-teal-600 text-white" : "hover:bg-gray-100"}`} onClick={() => onSelect(null)}>Không có biểu mẫu</div>
      {forms.map((f) => (
        <div key={f.formId} className={`px-3 py-2 rounded cursor-pointer text-sm ${selectedId === f.formId ? "bg-teal-600 text-white" : "hover:bg-gray-100"}`} onClick={() => onSelect(f.formId)}>{f.title}</div>
      ))}
    </div>
  )
}
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
            <input type="checkbox" checked={wantForm} onChange={(e) => { setWantForm(e.target.checked); if (!e.target.checked) onChange({ ...value, recruitmentFormId: null }) }} />
...
export default function OrgInfoPage() {
  ...
  const [recForm, setRecForm] = useState<RecruitmentPayload>({
    applyDeadline: null, responseDeadline: null, interviewSchedule: null, recruitmentFormId: null,
  })
  ...
  const editMut = useMutation({
    mutationFn: (data: { organizationName: string; description: string }) => updateMyOrganization(org!.organizationId, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["my-organization"] }); setShowEditDialog(false); toast.success("Cập nhật thông tin thành công") },
    onError: () => toast.error("Cập nhật thất bại"),
  })

  const openMut = useMutation({
    mutationFn: (data: RecruitmentPayload) => openRecruitment(org!.organizationId, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["my-organization"] }); setShowOpenDialog(false); toast.success("Đã mở đơn đăng ký") },
    onError: () => toast.error("Mở đơn thất bại"),
  })

  const closeMut = useMutation({
    mutationFn: () => closeRecruitment(org!.organizationId),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["my-organization"] }); toast.success("Đã đóng đơn đăng ký") },
    onError: () => toast.error("Đóng đơn thất bại"),
  })

  const updateMut = useMutation({
    mutationFn: (data: RecruitmentPayload) => updateRecruitment(org!.organizationId, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["my-organization"] }); setShowUpdateDialog(false); toast.success("Đã cập nhật đơn đăng ký") },
    onError: () => toast.error("Cập nhật thất bại"),
  })
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center min-h-[400px]"><div className="animate-pulse text-gray-400">Đang tải...</div></div>
  }
  ...
  return (
    <div>
      ...
      <div className="relative w-full h-[160px] sm:h-[220px] md:h-[300px] lg:h-[340px] overflow-hidden">
        <EditableImage src={coverSrc} fallbackSrc="/team-building.jpg" alt="cover" fill className="object-cover object-center" editable={isEditing} folder="covers" onUploaded={(key) => saveImageField("coverImageUrl", key)} />
      </div>

      {/* Logo + Name (single column, no left/right images for organization type) */}
      <div className="flex flex-col items-center py-5 sm:py-7 md:py-8">
        <div className="relative rounded-full overflow-hidden border-4 border-white shadow-lg w-[96px] h-[96px] sm:w-[130px] sm:h-[130px] md:w-[150px] md:h-[150px]">
          <EditableImage src={logoSrc} fallbackSrc="/logo-club.jpg" alt="logo" fill className="object-cover" editable={isEditing} folder="logos" rounded onUploaded={(key) => saveImageField("logoUrl", key)} />
        </div>
        <div className="text-[#1A73E8] text-[16px] sm:text-[20px] md:text-[24px] font-bold text-center leading-tight mt-3">
          {org?.organizationName ?? "—"}
        </div>
        <span className="text-xs text-gray-500 mt-1">
          {org?.organizationType === "organization" ? "Tổ chức" : "Câu lạc bộ"}
        </span>
      </div>

      {isEditing && (
        <p className="text-center text-xs text-teal-600 -mt-3 mb-4">Click vào ảnh để thay đổi</p>
      )}
      ...
      <div className="flex justify-center gap-3 sm:gap-4 mt-8 sm:mt-10 flex-wrap mb-10 px-3">
        {!org?.isRecruiting ? (
          <button className="bg-[#08667a] text-white px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-[#06505f] transition" onClick={handleOpenRecruitDialog}>
            Mở đơn đăng ký
          </button>
        ) : (
          <button className="bg-red-500 text-white px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-red-600 transition disabled:opacity-50" onClick={() => closeMut.mutate()} disabled={closeMut.isPending}>
            {closeMut.isPending ? "Đang đóng..." : "Đóng đơn đăng ký"}
          </button>
        )}

        <button className="bg-[#08667a] text-white px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-[#06505f] transition" onClick={handleClickEditInfo}>
          Chỉnh sửa thông tin
        </button>

        {!isEditing ? (
          <button className="border border-[#08667a] text-[#08667a] px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-[#08667a] hover:text-white transition" onClick={() => setIsEditing(true)}>
            Chỉnh sửa ảnh
          </button>
        ) : (
          <button className="bg-teal-600 text-white px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-teal-700 transition" onClick={() => setIsEditing(false)}>
            Xong chỉnh sửa ảnh
          </button>
        )}

        {org?.isRecruiting && (
          <button className="bg-[#1A73E8] text-white px-4 sm:px-6 py-2 rounded-full text-sm sm:text-base font-semibold hover:bg-[#1557b0] transition" onClick={handleOpenUpdateDialog}>
      ...
      <OverlayDialog open={showEditDialog} onClose={() => setShowEditDialog(false)}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input className="w-full border rounded-lg px-3 py-2 text-sm" value={editName} onChange={(e) => setEditName(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Mô tả</label>
            <textarea className="w-full border rounded-lg px-3 py-2 text-sm min-h-[100px]" value={editDesc} onChange={(e) => setEditDesc(e.target.value)} />
          </div>
        </div>
        <div className="flex justify-end gap-3 mt-4">
          <button className="px-4 py-2 rounded-lg border text-sm" onClick={() => setShowEditDialog(false)}>Hủy</button>
          <button className="px-4 py-2 rounded-lg bg-[#08667a] text-white text-sm disabled:opacity-50" disabled={editMut.isPending}
            onClick={() => editMut.mutate({ organizationName: editName, description: editDesc })}>
      ...
      {/* Recruitment dialogs */}
      <RecruitmentDialog open={showOpenDialog} onClose={() => setShowOpenDialog(false)} title="Mở đơn đăng ký" organizationId={org?.organizationId ?? 0} value={recForm} onChange={setRecForm} isPending={openMut.isPending} onSubmit={() => openMut.mutate(recForm)} />
      <RecruitmentDialog open={showUpdateDialog} onClose={() => setShowUpdateDialog(false)} title="Cập nhật đơn đăng ký" organizationId={org?.organizationId ?? 0} value={recForm} onChange={setRecForm} isPending={updateMut.isPending} onSubmit={() => updateMut.mutate(recForm)} />
    </div>
  )
}
